...
def stream_llm_response(context, query, repo_url, repo_context=None, repo_meta=None, oauth_user=None):

    llm_call_start_time = time.time()
    logging.info("Initiating LLM call to Azure OpenAI.")
    system_prompt = f"""# ROLE & GOAL
You are GitBro, a highly experienced software engineer and reviewer embedded in the GitForme app. Provide concise, accurate, and helpful analysis based *exclusively* on the provided context about the repository: {repo_url}. Do not use external knowledge.

# OAUTH & SECURITY GUIDANCE
- Explain OAuth flows clearly when asked (authorization code, callback, token exchange, session/JWT).
- Never request secrets or tokens from the user.
- If auth data is missing, say what is needed at a high level.
...
- **FORMATTING**"""
    try:
        ...
        context_blocks = [
            "Here is the context retrieved from the repository files:",
            context or "(no repo file context)",
        ]
        if repo_context:
            context_blocks.extend([
                "\nAdditional context from the app (LLM Context Builder):",
                repo_context
            ])
        if repo_meta:
            context_blocks.extend([
                "\nRepository data fetched via GitHub OAuth:",
                json.dumps(repo_meta, ensure_ascii=False)[:8000]
            ])
        if oauth_user:
            context_blocks.extend([
                "\nAuthenticated GitHub user:",
                json.dumps(oauth_user, ensure_ascii=False)[:2000]
            ])
...
async def chat():
    ...
    repo_meta = data.get("repoMeta")
    oauth_user = data.get("oauthUser")
