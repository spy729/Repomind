...
const LandingPageContent = () => {
    ...
    return (
        <motion.div initial="hidden" animate="visible" exit={{ opacity: 0, transition: { duration: 0.5 } }} className="min-h-screen">
            <AdBlockDetectedWrapper>
                <motion.div
                    variants={itemVariants}
                    className="max-w-4xl mx-auto mt-4 mb-6 p-4 glass border border-accent-300 text-white rounded-2xl shadow-medium"
                    role="alert"
                >
                    <div className="flex items-center">
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            className="h-6 w-6 mr-3 text-accent-300"
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                        >
         ...
         {/* <motion.div
            ...
            <main className="flex flex-col items-center justify-center pt-16 md:pt-24 px-4 text-center relative overflow-hidden">
                <div className="relative z-10">
                    <SparkleIcon className="absolute top-[-3rem] left-[-5rem] w-20 h-20 text-accent-300 rotate-[-15deg] opacity-60 -z-10 animate-float" />
                    <SparkleIcon className="absolute top-[-1rem] right-[-5rem] w-24 h-24 text-secondary-300 rotate-[20deg] opacity-60 -z-10 animate-float" style={{animationDelay: '3s'}} />

                    <motion.h1
                        variants={itemVariants}
                        className="text-5xl md:text-7xl font-bold tracking-tight font-inter text-secondary-300 text-shadow"
                    >
                        Dive into Open Source.
                    </motion.h1>
                    <motion.h1
                        variants={itemVariants}
                        className="text-5xl md:text-7xl font-bold tracking-tight mt-2 text-gradient pb-2 font-inter"
                    >
                    ...
                    <motion.p
                        variants={itemVariants}
                        className="mt-8 text-lg md:text-xl text-white/90 max-w-2xl mx-auto font-medium leading-relaxed"
                    >
                        Transform any GitHub repository into an <span className="font-bold text-secondary-300">AI-powered guide</span> for newcomers and seasoned developers alike. Get structured insights, understand code, and contribute with confidence.
                    </motion.p>
                </div>
            </main>
            ...
            <motion.section variants={sectionVariants} initial="hidden" animate="visible" className="py-24 px-4">
                <div className="text-center mb-16">
                    <motion.h2
                        variants={itemVariants}
                        className="text-4xl font-bold tracking-tight font-inter text-secondary-300 text-shadow"
                    >
            ...
            <motion.section variants={sectionVariants} initial="hidden" animate="visible" className="py-24 px-4 glass border-y border-white/30">
                <div className="max-w-6xl mx-auto">
                    <div className="text-center mb-16">
                        <motion.h2
                            variants={itemVariants}
                            className="text-4xl font-bold tracking-tight font-inter text-white text-shadow"
                        >
            ...
            <motion.div variants={itemVariants} className="mt-20 py-16 text-center glass border-y border-white/30">
                <h3 className="text-3xl font-bold text-white text-shadow mb-6 font-inter">Ready to Contribute?</h3>
                <p className="text-lg text-white/80 mb-8 max-w-2xl mx-auto">
                    Paste a GitHub repository URL in the header search bar and let GitForme illuminate your path.
                </p>
                {userCount !== null && (
                    <div className="inline-flex items-center gap-2 px-6 py-3 card-gradient rounded-full shadow-medium">
                        <div className="w-3 h-3 bg-accent-500 rounded-full animate-pulse"></div>
                        <span className="text-neutral-800 font-semibold">
                            {isCountLoading ? 'Loading...' : `${userCount.toLocaleString()}+ developers exploring`}
                        </span>
                    </div>
                )}
            </motion.div>
