...
const RepoDetailView = ({ onApiError, onRateLimitExceeded, onApiDown, llmContext, setLlmContext, setRepoMeta }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const repoRes = await apiClient.get(apiBase);
                const defaultBranch = repoRes.data.default_branch || 'main';

                const results = await Promise.allSettled([
                    apiClient.get(`${apiBase}/readme`),
                    apiClient.get(`${apiBase}/git/trees/${defaultBranch}?recursive=1`),
                    apiClient.get(`${apiBase}/contributors`),
                    apiClient.get(`${apiBase}/deployments`),
                    apiClient.get(`${apiBase}/issues`),
                    apiClient.get(`${apiBase}/good-first-issues`),
                    apiClient.get(`${apiBase}/insights`),
                    apiClient.get(`${apiBase}/hotspots`),
                    apiClient.get(`${apiBase}/insights/dependencies`),
                    apiClient.get(`${apiBase}/timeline`),
                    apiClient.get(`${apiBase}/pulls`),
                ]);
                ...
                const contributorsData = getData(results[2], []);
                const timelineDataResult = getData(results[9], null);
                const pullsData = getData(results[10], []);
                ...
                const issuesData = getData(results[4], { open: [], closed: [] });
                const goodFirstIssuesData = getData(results[5], []);
                const insightsData = getData(results[6], null);
                const hotspotsData = getData(results[7], []);
                const dependencyHealthData = getData(results[8], null);

                setIssues(issuesData);
                setGoodFirstIssues(goodFirstIssuesData);
                setInsights(insightsData);
                setHotspots(hotspotsData);
                setDependencyHealth(dependencyHealthData);
                ...
                if (typeof setRepoMeta === 'function') {
                    setRepoMeta({
                        repo: {
                        ...
                        counts: {
                            ...
                            good_first_issues: goodFirstIssuesData.length,
                            issues_open: issuesData?.open?.length || 0,
                            issues_closed: issuesData?.closed?.length || 0,
                            pulls: Array.isArray(pullsData) ? pullsData.length : 0,
                        },
                        insights: insightsData,
                        hotspots: hotspotsData.slice(0, 10),
                        issues_sample: (issuesData?.open || []).slice(0, 5),
                        pulls_sample: Array.isArray(pullsData) ? pullsData.slice(0, 5) : [],
                        timeline_summary: timelineDataResult ? {
                            commits: timelineDataResult.commits?.length || 0,
                            branches: timelineDataResult.branches?.length || 0,
                            tags: timelineDataResult.tags?.length || 0,
                        } : null,
                        dependency_summary: dependencyHealthData?.summary || null,
                    });
                }
    ...
    const handleFileSelect = useCallback(async (fileNode) => {
        ...
        try {
            const contentRes = await apiClient.get(`${apiBase}/contents/${fileNode.path}`);
