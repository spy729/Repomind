...
export const RepoHeaderCard = ({ username, reponame, repoData, deployments, onFastClone, onGenerateReport, isReportLoading, onAiChatClick }) => {
    ...
    return (
        <div className="card-gradient shadow-medium hover:shadow-large transition-all duration-300">
            <h1 className="text-4xl font-bold tracking-tight mb-3">
                <a href={`https://github.com/${username}`} target="_blank" rel="noopener noreferrer" className="hover:text-primary-600 transition-colors text-neutral-800">{username}</a> / 
                <a href={`https://github.com/${username}/${reponame}`} target="_blank" rel="noopener noreferrer" className="text-gradient hover:scale-105 inline-block transition-transform"> {reponame}</a>
            </h1>
            <p className="text-lg text-neutral-600 mb-5 leading-relaxed">{repoData.description}</p>
            <div className="flex flex-wrap items-center gap-x-6 gap-y-3 mb-6 text-neutral-700 text-lg">
            ...
            <div className="flex flex-wrap gap-3">
                {deployments && deployments.length > 0 && (
                    <div className="relative" ref={dropdownRef}>
                        ...
                        {isDropdownOpen && (
                            <div className="absolute top-full mt-2 w-max min-w-full glass border border-white/30 rounded-xl shadow-large z-10 overflow-hidden">
                                <ul className="divide-y divide-white/20">
                                    {deployments.map((dep) => (<li key={dep.url}><a href={dep.url} target="_blank" rel="noopener noreferrer" className="block w-full text-left px-4 py-3 hover:bg-white/20 transition-colors font-semibold text-base text-white"><span className="mr-2 capitalize">{dep.environment}</span></a></li>))}
                                </ul>
                            </div>
                        )}
                    </div>
                )}
                <button onClick={onFastClone} className="btn-accent flex items-center justify-center gap-2">🚀 FastClone</button>
                <button
                    onClick={onGenerateReport}
                    disabled={isReportLoading}
                    className="btn-primary disabled:bg-neutral-400 disabled:text-neutral-600 disabled:cursor-wait disabled:shadow-none disabled:scale-100 flex items-center justify-center gap-2"
                >
                    {isReportLoading ? (
                        <>
                ...
                <button onClick={onAiChatClick} className="btn-secondary flex items-center justify-center gap-2">💬 AI Chat</button>
            </div>
        </div>
    );
};
